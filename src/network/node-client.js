...
function connectToPeers(myNodeId, onMessage) {
  ...
  for (const peer of peers) {
    ...
    socket.on('open', () => { console.log(`[${myNodeId}] Connected to ${peer.id}`); });

    socket.on('message', (data) => { onMessage(peer.id, data.toString()); });

    socket.on('error', (err) => { console.log(`[${myNodeId}] Could not connect to ${peer.id}: ${err.code}`); });
